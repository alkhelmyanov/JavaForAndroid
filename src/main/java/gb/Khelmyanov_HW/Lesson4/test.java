...
public class test {
    ...
    public static void computerStep() {
        ...
        for (int y = 0; y < fieldSizeY; y++) {
            for (int x = 0; x < fieldSizeX; x++) {
                if (field[y][x] == DOT_EMPTY){
                    field[y][x] = DOT_COMPUTER;
                    if (checkWinByLine(DOT_COMPUTER, lineForWin))
                        return;
                    else
                        field[y][x] = DOT_EMPTY;
                }
        ...
        boolean f = checkWinByLine(DOT_HUMAN_1, lineForWin - 1);

        for (int y = 0; y < fieldSizeY; y++) {
            for (int x = 0; x < fieldSizeX; x++) {
                if (field[y][x] == DOT_EMPTY){
                    field[y][x] = DOT_HUMAN_1;
                    if (checkWinByLine(DOT_HUMAN_1, lineForWin - (f ? 0 : 1))) {
                        field[y][x] = DOT_COMPUTER;
                        return;
                    }
                    else
    ...
    public static void gameHumanToHuman() {
        ...
        do {
            ...
            humanOneStep();
            checkWinByLine(DOT_HUMAN_1, lineForWin);
            renderingField(field);

            humanTwoStep();
            checkWinByLine(DOT_HUMAN_2, lineForWin);
            renderingField(field);


        } while (!checkWinByLine(DOT_HUMAN_1, lineForWin) && !checkWinByLine(DOT_HUMAN_2, lineForWin));
    ...
    public static void gameHumanToComputer() {
        ...
        do {
            ...
            computerStep();
            checkWinByLine(DOT_COMPUTER, lineForWin);
            renderingField(field);
            //System.out.println(checkWinByLine(DOT_COMPUTER));
        } while (!checkWinByLine(DOT_HUMAN_1, lineForWin) && !checkWinByLine(DOT_COMPUTER, lineForWin));
    ...
    public static boolean checkWinByLine(char charOfPlayer, int lineForWin) {
    ...
    public static boolean rangeOfGameForPC(int x, int y) {
        return x >= 0 && y >= 0 && x <= fieldSizeX - 1 && y <= fieldSizeY - 1;
    }
